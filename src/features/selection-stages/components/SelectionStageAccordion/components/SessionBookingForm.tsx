...
export function SessionBookingForm({ 
  ...
  const stageRequiresSession = event ? requiresSessionFromEvent(event) : false;
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h5 className="font-medium text-base text-blue-600">
                {formatDateTime(sessionInfo.session.sessionDate)}
              </h5>
            ...
            {sessionInfo.session.maxParticipants && (
              <div className="flex items-center space-x-1 text-sm text-muted-foreground">
                ...
                <span>
                  {sessionInfo.session.maxParticipants}名
                </span>
              </div>
            )}
            {/* 参加状況を常に表示 */}
